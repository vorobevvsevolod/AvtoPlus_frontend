...
const YandexMap:React.FC<{categoryId?: string, workId?: number}> = (props) => {
    ...
    const { categories } = useSelector((root: RootState) => root.Category);
    ...
    const calculationCenterCardDefualt = (): {cordX: number, cordY:number} =>{
        const filteredWorks = galleryWorks.filter(work => work.categoryId === Number(props.categoryId));
    ...
    React.useEffect(() =>{
        if(categories.length && props.categoryId){
            const subCategories = categories.find(cat => cat.id === props.categoryId) ? categories.find(cat => cat.id === props.categoryId)?.sub : [];
